...
if __name__ == '__main__':
    ...
    tfile = 'todays_hits_' + str(pd.Timestamp.today().date()) + '.pkl'
    todays_hits = []
    with open(tfile, 'rb') as f:
        todays_hits = pickle.load(f)

    for ticker in todays_hits:
        """
        # options trading strategy
        1. Select an option that's 3 months away
        """
        ...
        for i in range(0, 4):
            ...
            logger.info("Call option quantity is - {}".format(quantity))
            if quantity <= 0:
